"""
Audio BSQ (Binary Spherical Quantization) model for audio tokenization.
Adapts BSQ-ViT for audio/speech processing.
"""
...
class AudioVQModel(nn.Module):
    """Audio Vector Quantization model."""
    def __init__(self, audioconfig, n_embed, embed_dim,
                 l2_norm=False, logit_laplace=False, ckpt_path=None, ignore_keys=[],
                 grad_checkpointing=False, selective_checkpointing=False,
                 clamp_range=(-80, 0),  # typical dB range for spectrograms
                 daudioconfig=None,
                 ):
        super().__init__()
        self.encoder = AudioTransformerEncoder(**audioconfig)
        daudioconfig = audioconfig if daudioconfig is None else daudioconfig
        self.decoder = AudioTransformerDecoder(**daudioconfig, logit_laplace=logit_laplace)
...
class AudioBSQModel(AudioVQModel):
    """Audio Binary Spherical Quantization model."""
    def __init__(self, audioconfig, embed_dim, embed_group_size=9,
                 l2_norm=False, logit_laplace=False, ckpt_path=None, ignore_keys=[],
                 grad_checkpointing=False, selective_checkpointing=False,
                 clamp_range=(-80, 0),
                 daudioconfig=None, beta=0., gamma0=1.0, gamma=1.0, zeta=1.0,
